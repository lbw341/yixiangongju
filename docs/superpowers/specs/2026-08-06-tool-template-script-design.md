...
3. 若存在脚本模板：
   - 将数据文件写入临时数据目录；
   - 执行 `python -u <脚本路径> <数据目录> <文件1> <文件2> ...`（沿用现有 `executePythonCode` 的 Python 探测、环境变量、超时、输出截断逻辑，但脚本来源改为工具模板路径，数据传递仅用命令行参数，不依赖 stdin）；
...
- 不再执行用户上传的 .py（消除任意代码执行风险）。
- 仅执行工具作者上传的脚本模板；脚本在独立临时工作目录内运行，沿用现有隔离方式。
